...
abstract class API
{
    ...
    public function __construct()
    {
        ...
        while ($this->args[0] == '' || $this->args[0] == 'useradmin-api') {
            while ($this->args[1] == '' || $this->args[1] == 'api.php'){
...
class MyAPI extends API
{
    ...
    protected function update_user()
    {
        $this->mh->setDbcoll('users');
        if ($this->method == "POST"){
            //if _id is an arguement than change the arguement to an int for MongoDB
            if(gettype($this->request["_id"])!="integer")
            {
                $this->request["_id"] = (int)$this->request["_id"];
            }
            //update the record at the _id passed in with new arguements
